...
// Stream 流式对话
func (c *OpenAIClient) Stream(ctx context.Context, messages []models.Message) (<-chan string, error) {
	// TODO: 实现流式响应
	return nil, fmt.Errorf("stream not implemented yet")
}

// convertMessages 转换消息格式
func (c *OpenAIClient) convertMessages(messages []models.Message) []openaiMessage {
	var result []openaiMessage
	for _, m := range messages {
		role := "user"
		if m.Type == models.MessageTypeAssistant {
			role = "assistant"
		} else if m.Type == models.MessageTypeSystem {
			role = "system"
		}
